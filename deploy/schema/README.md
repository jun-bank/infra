...
새 DB는 위 넷이면 된다. `04_alter_target_gateway.sql`은 **이미 적재된 기존 DB**용
마이그레이션이다 — 배포 대상 enum을 셋에서 넷으로 넓힌다(게이트웨이 추가 — ADR-027 DO-20
v0.5). 01의 정의가 이미 넷이므로 새 DB에는 적재하지 않아도 되고, 적재해도 결과는 같다.
