...
//   - 대상 범위는 사후조건과 같은 결박을 따른다(#21): DEPLOY_APP_SERVICE가 설정되면 그 app
//     서비스의 컨테이너만 본다(트레이드오프는 Executor.GreenContainers 주석 — 사이드카
//     재시작은 앱 준비성 판정과 무관하다). 결박이 없으면 프로젝트 전체를 보던 기존 범위다.
//
