...
// 프로젝트에서 파생해, 검사가 항상 실제 뜬 컨테이너를 대상으로 하게 한다. 빈 목록의 의미는
// 결박 여부에 따라 갈린다: 프로젝트 자체가 비었으면 빈 슬라이스(판정은 호출자 — health가
// 0개면 fail-closed로 닫는다), **결박이 켜졌는데 프로젝트에 컨테이너가 있으면서 대상 매치가
// 0개면 오류**다(서비스명 오설정·구조 변경 신호 — VerifyImageDigest ⑶과 같은 축, 무음 축소 금지).
//
